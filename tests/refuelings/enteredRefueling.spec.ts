    ...
    test('Создание обычной заявки', async ({ page }) => {
        ...
        await test.step('Создание заявки и запуск в работу', async () => {
            ...
            bidInfo = await bidFixture.ApiCommonBid({
                ...
                carFilter: `(isDeleted eq false and lastFixedAt le ${moment().subtract(2, 'd').format("YYYY-MM-DDTHH:mm:ss")}.000Z and lastFixedAt le ${moment().subtract(1, 'd').format("YYYY-MM-DDTHH:mm:ss")}.000Z)`,
                loadAddress: 'Челны',
                unloadAddress: 'Можга',
