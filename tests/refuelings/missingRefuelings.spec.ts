...
let bidInfo: any;
const outOfPlanning = [52.356187, 56.284728]
    ...
    test('пропуск АЗС', async ({ page }) => {
        ...
        await test.step('Планирование заправок по заявке ', async () => {
            ...
            if (bidInfoResponse.bidPoints.length > 2) {
                await emulatorApi.coordinatSend(bidInfo.carOption.carTracker, moment().subtract(1, 'd').format("YYYY-MM-DDTHH:mm:ss+03:00"), lastTrackerCarInfo[0].location.coordinates, [orderSort[0].geozone.location.coordinates, orderSort[1].geozone.location.coordinates, outOfPlanning],
                    [
                        { Number: 7, Address: 65535, Value: 200, ChangePer100Km: 0 },
                    ], "00:20:00", 80, moment().format("YYYY-MM-DDTHH:mm:ss+03:00"))
            }
            else {
                await emulatorApi.coordinatSend(bidInfo.carOption.carTracker, moment().subtract(1, 'd').format("YYYY-MM-DDTHH:mm:ss+03:00"), lastTrackerCarInfo[0].location.coordinates, [orderSort[0].geozone.location.coordinates, outOfPlanning],
                    [
                        { Number: 7, Address: 65535, Value: 200, ChangePer100Km: 0 },
                    ], "00:20:00", 80, moment().format("YYYY-MM-DDTHH:mm:ss+03:00"))
            }
            await page.waitForTimeout(310000); //ждём анализ координат
        })
        await test.step('Проверяю что определился факт пропуска заправки', async () => {
            await page.reload()
            await page.getByText('Показать пропущенные заправки').click();
            await expect(page.locator('[class="map-panel__head-title"]')).toContainText('Заправка Тестовая')
            // await expect(page.locator('[class="leaflet-marker-icon route-icon route-icon--refueling leaflet-zoom-animated leaflet-interactive"]').nth(1)).toBeVisible();
        })
        await test.step('Проверяем что в отчёте по АЗС будет информация по пропуску АЗС', async () => {
            ...
            await page.getByRole('option', { name: 'Пропустил' }).click();
