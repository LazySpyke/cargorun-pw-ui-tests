...
test.describe('Запрос GetCurrentBid', () => {
    ...
    test('Создание обычной заявки с перецепкой', async ({ page }) => {
        ...
        await test.step('Создание заявки и запуск в работу', async () => {
            ...
            await test.step('Проверка GetCurrentBid', async () => {
    ...
    test('Заявка с нагрузкой на ось', async ({ page }) => {
        await test.step('Логин', async () => {
            await loginPage.login(process.env.rootMail as string, process.env.rootPassword as string);
        });
        await test.step('Создание заявки и запуск в работу', async () => {
            ...
            await test.step('Отправка данных по нагрузке на ось', async () => {
                bidInfoResponse = await bidApi.GetBidInfo(bidResponse.id, await getAuthData(adminId));
                await emulatorApi.init();
                await emulatorApi.coordinatSend(bidInfo.carOption.carTracker, moment().subtract(7, 'd').format("YYYY-MM-DDTHH:mm:ss+00:00"), bidInfoResponse.bidPoints[0].geozone.location.coordinates, [bidInfoResponse.bidPoints[0].geozone.location.coordinates], [
            ...
            await test.step('Проверка GetCurrentBid', async () => {
                ...
                console.log(driverCurrentBidResponse)
                //TODo больше сравнений сделать по response, это пока временное быстрое решение
                if (driverCurrentBidResponse.axisLoad.value != 21 ||
                    driverCurrentBidResponse.axisLoad.secondaryValue != 22 ||
                    driverCurrentBidResponse.axisLoad.averageValue != 23 ||
                    driverCurrentBidResponse.axisLoad.trailerValue != 25) {
                    throw new Error('значения по машине нагрузке некорректная');
                }
            })
        });
    })
